...
void	move_collsion(t_vars *vars, int angle)
{
	...
	if (celldir_value[0] == '0' && celldir_value[1] == '0'
		&& celldir_value[2] == '0')
	{
		vars->perso.position[0] += movedist[0];
		vars->perso.position[1] += movedist[1];
	}
	else if (celldir_value[1] != '0' && dir[0] != 0 && dir[1] != 0
		&& !(celldir_value[0] == '0' && celldir_value[2] == '0'))
	{
		angle_direction_xy(angle, dir);
		if ((celldir_value[0] == '0' && dir[0] == dir[1])
			|| (celldir_value[2] == '0' && dir[0] != dir[1]))
			vars->perso.position[0] += movedist[0];
		else
			vars->perso.position[1] += movedist[1];
	}
...
void	set_move_dist(t_vars *vars, int angle, int movedist[2])
{
	...
	else
	{
		...
		movedist[0] = cos(angle * PI / 180) * PACE;
		movedist[1] = sin(angle * PI / 180) * PACE;
	}
}
